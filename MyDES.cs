public class MyDES
{
    public static string Encrypt(string plainText, string key, string iv)
    {
        byte[] data = Encoding.UTF8.GetBytes(plainText);
        byte[] ivBytes = Encoding.UTF8.GetBytes(iv);

        if (key.Length != 8 || iv.Length != 8)
            throw new Exception("Key and IV must be 8 characters.");

        // PKCS5 padding
        int pad = 8 - (data.Length % 8);
        byte[] padded = new byte[data.Length + pad];
        Array.Copy(data, padded, data.Length);
        for (int i = data.Length; i < padded.Length; i++)
            padded[i] = (byte)pad;

        byte[] result = new byte[padded.Length];
        byte[] previous = ivBytes;

        for (int i = 0; i < padded.Length; i += 8)
        {
            byte[] block = padded.Skip(i).Take(8).ToArray();

            // CBC XOR
            for (int j = 0; j < 8; j++)
                block[j] ^= previous[j];

            int[] encryptedBits = ProcessBits(BytesToBits(block), StringToBits(key), false);
            byte[] cipherBlock = BitsToBytes(encryptedBits);

            Array.Copy(cipherBlock, 0, result, i, 8);
            previous = cipherBlock;
        }

        return string.Join(":", result.Select(b => b.ToString("X2")));
    }

    public static string Decrypt(string cipherText, string key, string iv)
    {
        byte[] encryptedBytes = cipherText.Split(':')
                                          .Select(x => Convert.ToByte(x, 16))
                                          .ToArray();
        byte[] ivBytes = Encoding.UTF8.GetBytes(iv);
        byte[] result = new byte[encryptedBytes.Length];
        byte[] previous = ivBytes;

        for (int i = 0; i < encryptedBytes.Length; i += 8)
        {
            byte[] block = encryptedBytes.Skip(i).Take(8).ToArray();

            int[] decryptedBits = ProcessBits(BytesToBits(block), StringToBits(key), true);
            byte[] plainBlock = BitsToBytes(decryptedBits);

            // CBC XOR
            for (int j = 0; j < 8; j++)
                plainBlock[j] ^= previous[j];

            Array.Copy(plainBlock, 0, result, i, 8);
            previous = block;
        }

        int pad = result[result.Length - 1];
        return Encoding.UTF8.GetString(result, 0, result.Length - pad);
    }

    public static int[] ProcessBits(int[] blockBits, int[] keyBits, bool decrypt)
    {
        int[] bits = Permute(blockBits, IP);
        int[][] roundKeys = GenerateKeys(keyBits);
        if (decrypt) Array.Reverse(roundKeys);

        int[] L = new int[32];
        int[] R = new int[32];
        Array.Copy(bits, 0, L, 0, 32);
        Array.Copy(bits, 32, R, 0, 32);

        for (int i = 0; i < 16; i++)
        {
            int[] temp = R;
            R = XOR(L, F(R, roundKeys[i]));
            L = temp;
        }

        int[] combined = new int[64];
        Array.Copy(R, 0, combined, 0, 32);
        Array.Copy(L, 0, combined, 32, 32);

        return Permute(combined, FP);
    }

    public static int[] BytesToBits(byte[] bytes)
    {
        int[] bits = new int[bytes.Length * 8];
        for (int i = 0; i < bytes.Length; i++)
        {
            for (int j = 7; j >= 0; j--)
            {
                bits[i * 8 + (7 - j)] = (bytes[i] >> j) & 1;
            }
        }
        return bits;
    }

    public static byte[] BitsToBytes(int[] bits)
    {
        int len = bits.Length / 8;
        byte[] bytes = new byte[len];
        for (int i = 0; i < len; i++)
        {
            byte b = 0;
            for (int j = 0; j < 8; j++)
                b = (byte)((b << 1) | bits[i * 8 + j]);
            bytes[i] = b;
        }
        return bytes;
    }

    public static int[][] GenerateKeys(int[] key64)
    {
        int[][] roundKeys = new int[16][];
        int[] key56 = Permute(key64, PC1);

        int[] C = new int[28];
        int[] D = new int[28];
        Array.Copy(key56, 0, C, 0, 28);
        Array.Copy(key56, 28, D, 0, 28);

        for (int i = 0; i < 16; i++)
        {
            C = LeftShift(C, SHIFTS[i]);
            D = LeftShift(D, SHIFTS[i]);

            int[] combined = new int[56];
            Array.Copy(C, 0, combined, 0, 28);
            Array.Copy(D, 0, combined, 28, 28);

            roundKeys[i] = Permute(combined, PC2);
        }

        return roundKeys;
    }

    public static int[] F(int[] R, int[] K)
    {
        int[] ER = Permute(R, E);
        int[] X = XOR(ER, K);
        int[] outBits = new int[32];

        for (int i = 0; i < 8; i++)
        {
            int row = (X[i * 6] << 1) | X[i * 6 + 5];
            int col = (X[i * 6 + 1] << 3) | (X[i * 6 + 2] << 2) | (X[i * 6 + 3] << 1) | X[i * 6 + 4];
            int val = S[i, row, col];

            for (int j = 0; j < 4; j++)
                outBits[i * 4 + j] = (val >> (3 - j)) & 1;
        }

        return Permute(outBits, P);
    }

    public static int[] XOR(int[] a, int[] b)
    {
        int[] r = new int[a.Length];
        for (int i = 0; i < a.Length; i++) r[i] = a[i] ^ b[i];
        return r;
    }

    public static int[] Permute(int[] input, int[] table)
    {
        int[] output = new int[table.Length];
        for (int i = 0; i < table.Length; i++)
            output[i] = input[table[i] - 1];
        return output;
    }

    public static int[] LeftShift(int[] bits, int n)
    {
        int[] r = new int[bits.Length];
        Array.Copy(bits, n, r, 0, bits.Length - n);
        Array.Copy(bits, 0, r, bits.Length - n, n);
        return r;
    }

    public static int[] StringToBits(string input) => BytesToBits(Encoding.UTF8.GetBytes(input));
    public static string BitsToString(int[] bits) => Encoding.UTF8.GetString(BitsToBytes(bits));

    // ==== Таблици ====
    static int[] IP = { 58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4, 62, 54, 46, 38, 30, 22, 14, 6, 64, 56, 48, 40, 32, 24, 16, 8, 57, 49, 41, 33, 25, 17, 9, 1, 59, 51, 43, 35, 27, 19, 11, 3, 61, 53, 45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7 };
    static int[] FP = { 40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31, 38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13, 53, 21, 61, 29, 36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27, 34, 2, 42, 10, 50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25 };
    static int[] PC1 = { 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35, 27, 19, 11, 3, 60, 52, 44, 36, 63, 55, 47, 39, 31, 23, 15, 7, 62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 28, 20, 12, 4 };
    static int[] PC2 = { 14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10, 23, 19, 12, 4, 26, 8, 16, 7, 27, 20, 13, 2, 41, 52, 31, 37, 47, 55, 30, 40, 51, 45, 33, 48, 44, 49, 39, 56, 34, 53, 46, 42, 50, 36, 29, 32 };
    static int[] E = { 32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9, 10, 11, 12, 13, 12, 13, 14, 15, 16, 17, 16, 17, 18, 19, 20, 21, 20, 21, 22, 23, 24, 25, 24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1 };
    static int[] SHIFTS = { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 };
    static int[,,] S = new int[8, 4, 16]
    {
        { {14,4,13,1,2,15,11,8,3,10,6,12,5,9,0,7}, {0,15,7,4,14,2,13,1,10,6,12,11,9,5,3,8}, {4,1,14,8,13,6,2,11,15,12,9,7,3,10,5,0}, {15,12,8,2,4,9,1,7,5,11,3,14,10,0,6,13} },
        { {15,1,8,14,6,11,3,4,9,7,2,13,12,0,5,10}, {3,13,4,7,15,2,8,14,12,0,1,10,6,9,11,5}, {0,14,7,11,10,4,13,1,5,8,12,6,9,3,2,15}, {13,8,10,1,3,15,4,2,11,6,7,12,0,5,14,9} },
        { {10,0,9,14,6,3,15,5,1,13,12,7,11,4,2,8}, {13,7,0,9,3,4,6,10,2,8,5,14,12,11,15,1}, {13,6,4,9,8,15,3,0,11,1,2,12,5,10,14,7}, {1,10,13,0,6,9,8,7,4,15,14,3,11,5,2,12} },
        { {7,13,14,3,0,6,9,10,1,2,8,5,11,12,4,15}, {13,8,11,5,6,15,0,3,4,7,2,12,1,10,14,9}, {10,6,9,0,12,11,7,13,15,1,3,14,5,2,8,4}, {3,15,0,6,10,1,13,8,9,4,5,11,12,7,2,14} },
        { {2,12,4,1,7,10,11,6,8,5,3,15,13,0,14,9}, {14,11,2,12,4,7,13,1,5,0,15,10,3,9,8,6}, {4,2,1,11,10,13,7,8,15,9,12,5,6,3,0,14}, {11,8,12,7,1,14,2,13,6,15,0,9,10,4,5,3} },
        { {12,1,10,15,9,2,6,8,0,13,3,4,14,7,5,11}, {10,15,4,2,7,12,9,5,6,1,13,14,0,11,3,8}, {9,14,15,5,2,8,12,3,7,0,4,10,1,13,11,6}, {4,3,2,12,9,5,15,10,11,14,1,7,6,0,8,13} },
        { {4,11,2,14,15,0,8,13,3,12,9,7,5,10,6,1}, {13,0,11,7,4,9,1,10,14,3,5,12,2,15,8,6}, {1,4,11,13,12,3,7,14,10,15,6,8,0,5,9,2}, {6,11,13,8,1,4,10,7,9,5,0,15,14,2,3,12} },
        { {13,2,8,4,6,15,11,1,10,9,3,14,5,0,12,7}, {1,15,13,8,10,3,7,4,12,5,6,11,0,14,9,2}, {7,11,4,1,9,12,14,2,0,6,10,13,15,3,5,8}, {2,1,14,7,4,10,8,13,15,12,9,0,3,5,6,11} }
    };
    static int[] P = { 16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18, 31, 10, 2, 8, 24, 14, 32, 27, 3, 9, 19, 13, 30, 6, 22, 11, 4, 25 };
}
